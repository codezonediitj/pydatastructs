...
def test_depth_first_search():
    ...
    def _test_depth_first_search(ds):
        ...
        if (ds=='List'):
            ...
            G2.add_edge("9", "10")
            G2.add_edge("10", "11")
            depth_first_search(G2, "9", dfs_tree, parent, backend = Backend.CPP)
            assert parent[V10] == V9
            assert parent[V11] == V10
        ...
        if (ds == 'Matrix'):
            ...
            G3.add_edge("12", "13")
            G3.add_edge("13", "14")
            depth_first_search(G3, "12", dfs_tree, parent3, backend = Backend.CPP)
            assert parent3[V13] == V12
            assert parent3[V14] == V13
...
def test_shortest_paths():
    ...
    def _test_shortest_paths_positive_edges(ds, algorithm):
        ...
        if (ds == 'List' and algorithm == 'bellman_ford'):
            vertices3 = [AdjacencyListGraphNode('S', 0, backend = Backend.CPP),
                        AdjacencyListGraphNode('C', 0, backend = Backend.CPP),
                        AdjacencyListGraphNode('SLC', 0, backend = Backend.CPP),
                        AdjacencyListGraphNode('SF', 0, backend = Backend.CPP),
                        AdjacencyListGraphNode('D', 0, backend = Backend.CPP)]
            graph3 = Graph(*vertices3, backend = Backend.CPP)
            graph3.add_edge('S', 'SLC', 2)
            graph3.add_edge('C', 'S', 4)
            graph3.add_edge('C', 'D', 2)
            graph3.add_edge('SLC', 'C', 2)
            graph3.add_edge('SLC', 'D', 3)
            graph3.add_edge('SF', 'SLC', 2)
            graph3.add_edge('SF', 'S', 2)
            graph3.add_edge('D', 'SF', 3)
