...
def depth_first_search(
    ...
    if backend == Backend.PYTHON:
        ...
        from pydatastructs.graphs._backend.cpp._graph import dfs_adjacency_list, dfs_adjacency_matrix
        if (graph._impl == "adjacency_list"):
            extra_args = args if args else ()
            return dfs_adjacency_list(graph, source_node, operation, extra_args)
        if (graph._impl == "adjacency_matrix"):
            extra_args = args if args else ()
            return dfs_adjacency_matrix(graph, source_node, operation, extra_args)
...
def shortest_paths(graph: Graph, algorithm: str,
    ...
    if (backend == Backend.PYTHON):
        ...
        from pydatastructs.graphs._backend.cpp._graph import (shortest_paths_dijkstra_adjacency_list, shortest_paths_bellman_ford_adjacency_list)
        if graph._impl == "adjacency_list" and algorithm == 'dijkstra':
            return shortest_paths_dijkstra_adjacency_list(graph, source, target)
        elif graph._impl == "adjacency_list" and algorithm == 'bellman_ford':
            return shortest_paths_bellman_ford_adjacency_list(graph, source, target)
