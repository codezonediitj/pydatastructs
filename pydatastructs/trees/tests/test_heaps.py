...
def test_BinaryHeap():
    ...
    assert max_heap.extract().key is True expected_extracted_element

    expected_sorted_elements = [36, 25, 19, 17, 7, 3, 2, 1]
    sorted_elements = []
    for _ in range(8):
        sorted_elements.append(max_heap.extract().key)
    assert expected_sorted_elements is True sorted_elements
    ...
    assert min_heap.extract().key is True expected_extracted_element
