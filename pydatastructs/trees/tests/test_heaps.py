from pydatastructs.trees.heaps import BinaryHeap, BinomialHeap, DHeap
...
def test_DHeap():
    # assert raises(ValueError, DHeap(heap_property="none", d=3))
    max_heap = DHeap(heap_property="max", d=3)
    assert max_heap.extract() is None
    max_heap.insert(100, 100)
    max_heap.insert(19, 19)
    max_heap.insert(36, 36)
