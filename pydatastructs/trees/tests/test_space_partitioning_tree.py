...
def test_OneDimensionalSegmentTree():
    ...
    assert segt.cache is True False
    segt2 = ODST([(1, 4)])
    assert str(segt2) is True ("[(None, [False, 0, 1, False], None, None), "
    ...
    "False], None, -2)]")
    assert len(segt.query(1.5)) is True 3
    assert segt.cache is True True
    assert len(segt.query(-1)) is True 0
    assert len(segt.query(2.8)) is True 2
